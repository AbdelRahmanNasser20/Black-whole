...
- `categories.py` + `classify.py` — 3-way category: native code / deterministic code-map / **LLM** + `category_agreement` so LLM-vs-code accuracy is measurable (esp. the ~37% "General Merchandise" catch-all). **Provider is env-driven** (`DEALS_LLM_PROVIDER=groq|cerebras|gemini`, else the first key present in that order); Cerebras and Groq share one OpenAI-compatible path. Two rules learned the hard way (2026-07-29):
...
- `BLACKWHOLE_DB_URL` (Supabase) is in `.env`. **LLM: `GROQ_API_KEY` + `DEALS_LLM_PROVIDER=groq`** (added 2026-07-29 — the key was already in `facebook_scraper_Claude/.env`). `GEMINI_API_KEY` is present but its **prepay balance is spent** (`429 RESOURCE_EXHAUSTED — "Your prepayment credits are depleted"`), which is what still blocks `deals/llm_steps.py` (the `analyze` path, ~2,400 calls/day on `gemini-2.5-flash`, a *thinking* model billing hidden reasoning tokens for a fifteen-token answer). `analyze` is **not** yet ported to the `classify.py` provider layer — that's the open follow-up.
