...
- Auction expiry/relist sync: `.venv/bin/python scripts/sync_auction_status.py --once [--dry-run] [--lot ID]`; the loop runs in the web process (`AUCTION_SYNC_INTERVAL_SEC`=1800, `AUCTION_SYNC_ENABLED`=1). Needs `scripts/sql/011_inventory_auction_watch.sql` applied (**operator gate**).
